...
export default function SimplifyPrompt() {
  ...
  const getResponse = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newPrompt = `Simplify this text in ${wordnum} words: ${topic}`;

    console.log("newPrompt", newPrompt);

    const systemPrompt = "Simplify this topic";
    let finishedSummary = "";
      ...
      <form onSubmit={(e) => getResponse(e)}>
        <label htmlFor="topic-field">
          ...
          <textarea
            id="topic-field"
            rows={4}
            placeholder="Enter a paragraph that you would like to rewrite in a simpler form"
