...
export async function POST(req: NextRequest) {
  try {
    ...
    const fireworksAPIKey = requestFormData.get("fireworksAPIKey") as
      | string
      | null;
    const stabilityAPIKey = requestFormData.get("stabilityAPIKey") as
      | string
      | null;
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          error: `Error from Image API: ${response.status} ${response.statusText}`,
        },
        { status: 500 }
      );
    }
